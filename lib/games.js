import { getSupabase } from './supabase.js';
import { getCurrentUserId } from './auth.js';

function scoreFromRounds(game) {
  const out = { t1: 0, t2: 0 };
  (game.rounds || []).forEach((r) => {
    out[r.team] = (out[r.team] || 0) + r.points;
  });
  return out;
}

const UI_TO_DB_FORMAT = {
  teteATete: 'tete_a_tete',
  doublette: 'doublette',
  triplette: 'triplette',
  unContreDeux: 'un_contre_deux',
};

function dbFormatToUi(db) {
  const m = {
    tete_a_tete: 'teteATete',
    doublette: 'doublette',
    triplette: 'triplette',
    un_contre_deux: 'unContreDeux',
  };
  return m[db] || 'doublette';
}

function formatDateFr(iso) {
  if (!iso) return '—';
  const d = new Date(iso);
  return d.toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }).toUpperCase();
}

function formatTimeFr(iso) {
  if (!iso) return '—';
  return new Date(iso).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
}

/** Normalise une ligne API (nested select) vers le modèle UI du prototype */
export function mapRemoteGame(row) {
  if (!row) return null;
  const teamsJoined = row.teams ?? row['teams!teams_game_id_fkey'];
  const teamsRaw = [...(teamsJoined || [])].sort((a, b) => a.position - b.position);
  const teams = teamsRaw.map((t, idx) => {
    const side = idx === 0 ? 't1' : 't2';
    const sortedPlayers = [...(t.players || [])].sort((a, b) => a.sort_order - b.sort_order);
    const players = sortedPlayers.map((p) => p.name);
    const playerRefs = sortedPlayers.map((p) => ({ name: p.name, userId: p.user_id || null }));
    return {
      id: side,
      dbId: t.id,
      name: t.name,
      color: t.color,
      players,
      playerRefs,
    };
  });

  const roundsRaw = [...(row.rounds || [])].sort((a, b) => a.round_index - b.round_index);
  const teamIdToSide = {};
  teamsRaw.forEach((t, i) => {
    teamIdToSide[t.id] = i === 0 ? 't1' : 't2';
  });
  const rounds = roundsRaw.map((r) => ({
    team: teamIdToSide[r.team_id] || 't1',
    points: r.points,
  }));

  const status = row.status === 'finished' || row.status === 'cancelled' ? 'archived' : 'live';
  const sc = { t1: 0, t2: 0 };
  rounds.forEach((r) => {
    sc[r.team] = (sc[r.team] || 0) + r.points;
  });

  let winner = null;
  if (row.status === 'finished' && row.winner_team_id && teamsRaw.length >= 2) {
    winner = row.winner_team_id === teamsRaw[0].id ? 't1' : 't2';
  }

  return {
    id: row.id,
    name: row.name,
    place: row.place || '',
    date: formatDateFr(row.finished_at || row.started_at || row.created_at),
    startedAt: formatTimeFr(row.started_at || row.created_at),
    format: dbFormatToUi(row.format),
    target: row.target,
    best_of: row.best_of,
    status,
    teams,
    rounds,
    spectators: row.spectator_count ?? 0,
    winner,
    finalScore: row.status === 'finished' ? [sc.t1, sc.t2] : null,
    eventId: row.event_id ?? null,
    _raw: row,
  };
}

const GAME_SELECT = `
  id, name, place, format, target, best_of, status, started_at, finished_at, winner_team_id,
  spectator_count, created_at, event_id,
  teams!teams_game_id_fkey ( id, name, color, position, players ( id, name, sort_order, user_id ) ),
  rounds ( id, team_id, points, round_index )
`;

export async function fetchGames() {
  const sb = getSupabase();
  const { data, error } = await sb
    .from('games')
    .select(GAME_SELECT)
    .order('created_at', { ascending: false });

  if (error) throw error;
  return (data || []).map(mapRemoteGame);
}

export async function fetchPlayerNames() {
  const sb = getSupabase();
  const { data, error } = await sb
    .from('players')
    .select('name')
    .order('name', { ascending: true });
  if (error) throw error;
  const seen = new Set();
  const out = [];
  for (const row of data || []) {
    const n = (row.name || '').trim();
    if (!n) continue;
    const key = n.toLowerCase();
    if (seen.has(key)) continue;
    seen.add(key);
    out.push(n);
  }
  return out;
}

export async function fetchGameById(gameId) {
  const sb = getSupabase();
  const { data, error } = await sb
    .from('games')
    .select(GAME_SELECT)
    .eq('id', gameId)
    .maybeSingle();

  if (error) throw error;
  return data ? mapRemoteGame(data) : null;
}

/**
 * Crée une partie live + 2 équipes + joueurs.
 * @param {{ name, place, formatUi, target, bestOf, teamA: {name,color,players[]}, teamB: {name,color,players[]} }} payload
 */
export async function createGameRemote(payload) {
  const sb = getSupabase();
  const uid = await getCurrentUserId();
  if (!uid) throw new Error('Not authenticated');
  const dbFormat = UI_TO_DB_FORMAT[payload.formatUi] || 'doublette';
  const now = new Date().toISOString();

  const { data: gameRow, error: gErr } = await sb
    .from('games')
    .insert({
      owner_id: uid,
      name: payload.name,
      place: payload.place || null,
      format: dbFormat,
      target: payload.target,
      best_of: payload.bestOf > 1 ? payload.bestOf : null,
      status: 'live',
      started_at: now,
      spectator_count: 0,
      event_id: payload.eventId || null,
    })
    .select('id')
    .single();

  if (gErr) throw gErr;
  const gameId = gameRow.id;

  const sides = [
    { position: 1, ...payload.teamA },
    { position: 2, ...payload.teamB },
  ];

  for (const side of sides) {
    const { data: teamRow, error: tErr } = await sb
      .from('teams')
      .insert({
        game_id: gameId,
        name: side.name,
        color: side.color,
        position: side.position,
      })
      .select('id')
      .single();
    if (tErr) throw tErr;

    const playerRows = (side.players || []).map((p, i) => {
      const isObject = p && typeof p === 'object';
      const name = (isObject ? p.name : p) || `Joueur ${i + 1}`;
      const userId = isObject ? (p.userId || null) : null;
      return {
        team_id: teamRow.id,
        name,
        user_id: userId,
        sort_order: i,
      };
    });
    if (playerRows.length) {
      const { error: pErr } = await sb.from('players').insert(playerRows);
      if (pErr) throw pErr;
    }
  }

  return fetchGameById(gameId);
}

export async function addRoundRemote(gameId, teamSide, points) {
  const sb = getSupabase();
  const game = await fetchGameById(gameId);
  if (!game) throw new Error('Game not found');
  const team = teamSide === 't1' ? game.teams[0] : game.teams[1];
  if (!team?.dbId) throw new Error('Invalid team');

  const nextIdx = (game.rounds || []).length;

  const { error } = await sb.from('rounds').insert({
    game_id: gameId,
    team_id: team.dbId,
    points,
    round_index: nextIdx,
  });
  if (error) throw error;
  return fetchGameById(gameId);
}

export async function undoLastRoundRemote(gameId) {
  const sb = getSupabase();
  const { data: last, error: qErr } = await sb
    .from('rounds')
    .select('id, round_index')
    .eq('game_id', gameId)
    .order('round_index', { ascending: false })
    .limit(1)
    .maybeSingle();
  if (qErr) throw qErr;
  if (!last) return fetchGameById(gameId);

  const { error: dErr } = await sb.from('rounds').delete().eq('id', last.id);
  if (dErr) throw dErr;
  return fetchGameById(gameId);
}

export async function finalizeGameRemote(gameId) {
  const sb = getSupabase();
  const game = await fetchGameById(gameId);
  if (!game || game.status !== 'live') return game;

  const sc = scoreFromRounds(game);
  if (sc.t1 < game.target && sc.t2 < game.target) return game;

  let winnerTeamDbId = null;
  if (sc.t1 > sc.t2) winnerTeamDbId = game.teams[0].dbId;
  else if (sc.t2 > sc.t1) winnerTeamDbId = game.teams[1].dbId;

  const { error } = await sb
    .from('games')
    .update({
      status: 'finished',
      finished_at: new Date().toISOString(),
      winner_team_id: winnerTeamDbId,
    })
    .eq('id', gameId)
    .eq('status', 'live');

  if (error) throw error;
  return fetchGameById(gameId);
}

export async function deleteGameRemote(gameId) {
  const sb = getSupabase();
  const { error } = await sb.from('games').delete().eq('id', gameId);
  if (error) throw error;
}

// ────────────── Events ──────────────

function mapEventRow(row) {
  if (!row) return null;
  return {
    id: row.id,
    name: row.name,
    place: row.place || '',
    status: row.status === 'archived' ? 'archived' : 'live',
    inviteToken: row.invite_token || null,
    ownerId: row.owner_id || null,
    startedAt: formatTimeFr(row.started_at || row.created_at),
    date: formatDateFr(row.finished_at || row.started_at || row.created_at),
    started_at: row.started_at,
    finished_at: row.finished_at,
    created_at: row.created_at,
  };
}

const EVENT_SELECT = 'id, name, place, status, owner_id, invite_token, started_at, finished_at, created_at';

export async function fetchEvents() {
  const sb = getSupabase();
  const { data, error } = await sb
    .from('events')
    .select(EVENT_SELECT)
    .order('created_at', { ascending: false });
  if (error) throw error;
  return (data || []).map(mapEventRow);
}

export async function fetchEventById(eventId) {
  const sb = getSupabase();
  const { data, error } = await sb
    .from('events')
    .select(EVENT_SELECT)
    .eq('id', eventId)
    .maybeSingle();
  if (error) throw error;
  return data ? mapEventRow(data) : null;
}

export async function createEventRemote({ name, place }) {
  const sb = getSupabase();
  const uid = await getCurrentUserId();
  if (!uid) throw new Error('Not authenticated');
  const { data, error } = await sb
    .from('events')
    .insert({
      owner_id: uid,
      name: name?.trim() || 'Événement',
      place: place?.trim() || null,
      status: 'live',
      started_at: new Date().toISOString(),
    })
    .select(EVENT_SELECT)
    .single();
  if (error) throw error;
  return mapEventRow(data);
}

export async function finalizeEventRemote(eventId) {
  const sb = getSupabase();
  const { data, error } = await sb
    .from('events')
    .update({
      status: 'archived',
      finished_at: new Date().toISOString(),
    })
    .eq('id', eventId)
    .eq('status', 'live')
    .select(EVENT_SELECT)
    .maybeSingle();
  if (error) throw error;
  return data ? mapEventRow(data) : null;
}

export async function deleteEventRemote(eventId) {
  const sb = getSupabase();
  // games' event_id is ON DELETE SET NULL — orphaned games stay around as standalone.
  const { error } = await sb.from('events').delete().eq('id', eventId);
  if (error) throw error;
}

// ────────────── Realtime ──────────────

export function subscribeRounds(gameId, onChange) {
  const sb = getSupabase();
  const channel = sb
    .channel(`rounds:${gameId}`)
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'rounds', filter: `game_id=eq.${gameId}` },
      () => {
        fetchGameById(gameId).then(onChange).catch(() => {});
      },
    )
    .subscribe();

  return () => {
    sb.removeChannel(channel);
  };
}

/**
 * Realtime sync for an event: any change to its games (insert / update /
 * delete) or to any round (which we filter client-side by game_id) triggers
 * `onChange`, which the caller typically wires to a refresh of fetchGames().
 */
export function subscribeEvent(eventId, onChange) {
  const sb = getSupabase();
  const trigger = () => { try { onChange?.(); } catch { /* swallow */ } };

  const channel = sb
    .channel(`event:${eventId}`)
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'games', filter: `event_id=eq.${eventId}` },
      trigger,
    )
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'rounds' },
      trigger,
    )
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'events', filter: `id=eq.${eventId}` },
      trigger,
    )
    .subscribe();

  return () => {
    sb.removeChannel(channel);
  };
}

/**
 * Coarse-grained realtime sync used by the Home screen so the events and
 * games lists evolve without a manual reload. Postgres realtime respects the
 * current session's RLS, so the callback only fires for rows the visitor can
 * actually see.
 */
export function subscribeAll(onChange) {
  const sb = getSupabase();
  const trigger = () => { try { onChange?.(); } catch { /* swallow */ } };
  const channel = sb
    .channel('home:all')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'events' }, trigger)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'games' }, trigger)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'event_members' }, trigger)
    .subscribe();
  return () => {
    sb.removeChannel(channel);
  };
}
